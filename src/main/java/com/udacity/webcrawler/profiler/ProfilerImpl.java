...
final class ProfilerImpl implements Profiler {
  ...
  public Boolean profiledClassCheck(Class<?> klass) {
    List<Method> declaredMethods = new ArrayList<>(Arrays.asList(klass.getDeclaredMethods()));
    if (declaredMethods.isEmpty()) {
      return true;
    }
    return (!declaredMethods.stream().anyMatch(method -> method.getAnnotation(Profiled.class) != null));
  }
  ...
  public <T> T wrap(Class<T> klass, T delegate) {
    ...
    if (profiledClassCheck(klass)) {
      throw new IllegalArgumentException("Class " + klass.getName() + "does not have profiled methods");
    }
